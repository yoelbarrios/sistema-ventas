...
<main class="w-full overflow-y-auto">
          <div class="px-6 py-4 mx-auto grid">
            ...
            <div class="flex items-center px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              <!--start btn agregar-->
              <div class="my-2">
                <a href="{{url('user/create')}}" class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                  Agregar
                  ...
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                      ...
                      ******
                      ...
                      <td class="px-4 py-3 text-sm">
                        <img src="{{asset('storage/img/usuario/'.$user->imagen)}}" id="imagen1" alt="{{$user->name}}" class="img-responsive" width="60px" height="60px">
                      </td>
                      <td class="px-4 py-3">
                    ...
                    <ul class="inline-flex items-center">
                      <!--
                        <li class="@if($currentPage==$firstPage){{'disabled'}}@endif">
                            <a href="@if($currentPage>1){{$usuarios->url($firstPage)}}@else{{'#'}}@endif"
                         ...
                         -->
                      <li class="@if($currentPage==$firstPage){{'disabled'}}@endif">
                        <a href="@if($currentPage>1){{$usuarios->url($forwardPage)}}@else{{'#'}}@endif"
                        ...
                        @for($x=$currentPage;$x<$maxPages;$x++)
                            ...
                            <li class="@if($x==$currentPage){{'active'}}@endif">
                                    <a href="{{$usuarios->url($x)}}" class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple">{{$x}}</a>
                            </li>
                            @endif
                        @endfor
                      ...
                      <li class="@if($currentPage==$lastPage){{'disabled'}}@endif">
                        <a href="@if($currentPage<$lastPage){{$usuarios->url($nextPage)}}@else{{'#'}}@endif"
                      ...
                      <!--
                      <li class="@if($currentPage==$lastPage){{'disabled'}}@endif">
                        <a href="@if($currentPage<$lastPage){{$usuarios->url($lastPage)}}@else{{'#'}}@endif"
                          class="px-3 py-1 rounded-md focus:outline-none focus:shadow-outline-purple"
                        >
                          Ultima
                      </a>
