...
<main class="h-full overflow-y-auto">
          <div class="container px-6 py-4 mx-auto grid">
            ...
            <div class="flex items-center px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
              <!--start btn agregar-->
              <div class="my-2">
                <a href="{{url('categoria/create')}}" class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
                  Agregar
                    ...
                    <tr class="text-gray-700 dark:text-gray-400">
                      <td class="px-4 py-3">
                        <div class="flex items-center text-sm">
                          <div>
                            <p class="font-semibold">{{$cat->nombre}}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td class="px-4 py-3">
                        <div class="flex items-center space-x-4 text-sm">
                          <a href="{{url('/categoria/'.$cat->id.'/edit')}}"
                          ...
                          <form action="{{url('/categoria/'.$cat->id)}}" method="POST">
                              @csrf
                              {{method_field('DELETE')}}
                    ...
                    <ul class="inline-flex items-center">
                      <!--
                        <li class="@if($currentPage==$firstPage){{'disabled'}}@endif">
                            <a href="@if($currentPage>1){{$categorias->url($firstPage)}}@else{{'#'}}@endif"
                         ...
                         -->
                      <li class="@if($currentPage==$firstPage){{'disabled'}}@endif">
                        <a href="@if($currentPage>1){{$categorias->url($forwardPage)}}@else{{'#'}}@endif"
                        ...
                        @for($x=$currentPage;$x<$maxPages;$x++)
                            ...
                            <li class="@if($x==$currentPage){{'active'}}@endif">
                                    <a href="{{$categorias->url($x)}}" class="px-3 py-1 rounded-md rounded-r-lg focus:outline-none focus:shadow-outline-purple">{{$x}}</a>
                            </li>
                            @endif
                        @endfor
                      ...
                      <li class="@if($currentPage==$lastPage){{'disabled'}}@endif">
                        <a href="@if($currentPage<$lastPage){{$categorias->url($nextPage)}}@else{{'#'}}@endif"
                      ...
                      <!--
                      <li class="@if($currentPage==$lastPage){{'disabled'}}@endif">
                        <a href="@if($currentPage<$lastPage){{$categorias->url($lastPage)}}@else{{'#'}}@endif"
                          class="px-3 py-1 rounded-md focus:outline-none focus:shadow-outline-purple"
                        >
                          Ultima
                      </a>
